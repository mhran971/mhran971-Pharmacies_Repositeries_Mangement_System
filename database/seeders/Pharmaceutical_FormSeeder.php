...
class Pharmaceutical_FormSeeder extends Seeder
{
    ...
    {
        $files = Storage::disk('public')->files('TypesOfMedicine');
        ...
        $imageFiles = collect(Storage::disk('public')->files('TypesOfMedicine'))
        ...
        foreach ($pharmaceutical_form as &$pharmform) {
            $normalizedName = str_replace(' ', '_', $pharmform['name_en']);
            $pharmform['image_path'] = $imageFiles[$normalizedName] ?? null;
        }

        $pharmaceutical_form = array_filter($pharmaceutical_form, fn($lab) => $lab['image_path'] !== null);
