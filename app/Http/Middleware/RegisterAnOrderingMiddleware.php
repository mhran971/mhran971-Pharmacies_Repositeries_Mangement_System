...
class RegisterAnOrderingMiddleware
{
    /**
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($repoUser) {
            $hasPermission = $repoUser->user_repo_permission()->where('permission_id', 20)->exists();
        }
        ...
        if (!$hasPermission) {
            ...
            if ($pharmacyUser) {
                $hasPermission = $pharmacyUser->user_pharmacy_permissions()->where('permission_id', 20)->exists();
            }
        }
